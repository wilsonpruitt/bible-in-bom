...
3. ⚠️ **The criterion was wrong, and is now restated.** Frederick was acquired
   ...
   **Correction, 2026-09-07 (3 Nephi complete):** "3 Nephi 23" above was also
   wrong, the same kind of planning-session guess as the Alma one. Checked
   directly against `data/frederick-refs.json` and `text/frederick-clean.txt`
   while adjudicating 3 Nephi 23: zero rows there, and a five-form grep for
   "3 Nephi 23" plus its actual content (Samuel the Lamanite, "many saints",
   27:52, Helaman 14) found nothing. His real 3 Nephi rows are chapters 1, 5,
   9, 11, 19, 20, 30 — concentrated at 19 (the John 17 prayer, his genuine
   subject) — none of them chapter 23.
...
- **2026-09-07, Helaman bootstrapped, and a real parser bug caught before it reached the corpus.** `tools/parse-hardy.py --book "Helaman" --next-heading "The Third Book of Nephi"` ran without error and reported 101 Hardy rows — but `--next-heading` was wrong: Hardy's MSI appendix labels that section's own heading line just `Third Nephi`, not `The Third Book of Nephi` (which never occurs as its own line in the source). With no matching next-heading, `extract_section` had no bound and Helaman's "section" ran to the end of the entire file, so the walker kept going past Helaman 16:25 into 3 Nephi's own footnotes and mis-keyed roughly 90 of them onto Helaman 16:1-16:25 (verified: the Matthew 5:18/20/22/23/25 ↔ Helaman 16:18/20/22/23/25 correspondence is exact, and every other row traces to an identifiable line in 3 Nephi 12/20/24). The dispatched chapter-16 agent caught it independently before this was diagnosed here — it noticed the citations described a chapter that doesn't match what chapter 16 actually says, correctly called it a parser artifact, and did the right defensive thing (one anchor exclusion with the evidence, not 90 padded rows).

  Re-ran with `--next-heading "Third Nephi"`. Corrected apparatus: **8 verses, 11 citations** (not 101/31) — Helaman genuinely has almost no Bible apparatus outside chapters 6, 8, 12, and one row at 15:4; the audit now returns 0 chapter-shift suspects and 0 window-scan suspects (was 46, all of them chapter 16 bleed scoring against its neighbors, not real thematic ambiguity as first guessed). Verified by set-diff against the pre-bug baseline that every other book's `hardy-refs.json`/`hardy-recurrence.json` rows are byte-identical — the bug never wrote outside Helaman's own keys. The chapter-16 agent's first pass is discarded and re-dispatched against the corrected apparatus (zero Hardy rows for that chapter); chapters 1-15 were adjudicated against citations that were already correct, so nothing else needs redoing.

  **The lesson, now written into `ADJUDICATING.md` §0a:** the section-heading argument to `parse-hardy.py` needs the SAME kind of verification the running-head walker itself gets — grep the raw text for the literal `--next-heading` string as its own line BEFORE trusting a clean exit code. A wrong heading doesn't error; it just silently swallows everything after it.

- **2026-09-07 (Opus), Helaman complete: all 16 chapters, 200 links across 179 of 497 verses, 162 exclusions, all 11 (corrected) Hardy rows discharged, 0 gaps.** Six batched agents (five three-chapter batches plus a solo chapter 16, all dispatched in parallel) ran against the corrected apparatus; both gates return 0 errors on the merged book. The chapter-16 agent's second pass (against 0 real Hardy rows, as corrected) independently reproduced several of the book's strongest findings on machine candidates and its own reading alone, and wired its own `recurrenceRefs` back into chapters 13-14 without being told to. Wired into `data/books.ts` and the site; `next build` clean with all ten completed books live.

  **Findings that carry the book:** the Gadianton band is introduced in Proverbs 1:10-19's own vocabulary, six verses before its founding murder repeats Proverbs' "lay wait for their own blood" (2:1-6); Helaman 3's regnal formula (3:37) fuses two 2 Chronicles singletons as the positive twin of Mosiah 11:1's apostate one; 4:24-25 returns to Mosiah 1:13's own Samson allusion rather than the KJV directly, on "matchless," a word in no King James verse; 5:32's "Repent ye: for the kingdom" is an eight-word verbatim singleton of Matthew 3:2, the one place in the batch the n-gram stream ranked the true source first; 7:10 reads as a structural précis of Habakkuk 1-2 rather than a single borrowed phrase; 8:4's "for they feared the people, lest they should" is nine consecutive words confined to Acts 5:26 alone; 9:21 interleaves Acts 7:51 and Matthew 23:17 in Stephen's own charge; 10:4-7 reproduces Matthew 16:17-19 in sequence but the binding clause's pronoun is Matthew 18:18's plural, not 16:19's singular; 11 runs Elijah's drought (1 Kings 17-18) and Jonah's Nineveh in parallel; 12 rests its prosperity-thesis on Deuteronomy 8:13-14, three counts converging on one verse; 13:25's 1830 reading ("ye would not have slain the prophets") is the King James pronoun that 1920 corrected TOWARD, not away from — the rare case where the later edition restores rather than drifts from the source; 14:25 reproduces two consecutive Matthew 27:52-53 singletons in one sentence; and 16 closes the book answering its own signs: 16:14 fuses Luke's shepherds with Matthew's wise men in one clause, 16:16's "great and marvellous" returns to the same six words Lehi sang at 1 Nephi 1:14, and 16:23's "wrought among the people" is Acts 5:12's apostolic-growth clause with the outcome reversed into Satan's hold.

- **2026-09-07 (Opus), 3 Nephi complete: all 30 chapters, 534 links across 465 of 618 verses, 291 exclusions, all 89 Hardy rows discharged, 0 gaps.** Chapters 22, 24 and 25 (Isaiah 54, Malachi 3, Malachi 4) ran mechanically through `collate-isaiah.py`/`build-isaiah-adjudication.py`; the other 27 ran as dispatched Opus agents, one per chapter, all launched in parallel across the session; every one returned both gates clean on its first pass. Both gates clean on the merged book file (3 pre-existing lint warnings, all self-verified word-for-word against Hardy's own footnotes; 0 errors). Two real fixes preceded the pass: `parse-hardy.py`'s footnote-paragraph classifier was misreading a body paragraph with 2+ inline superscript-marker-shaped substrings as a footnote block (found on 3 Nephi 2, silently dropping verses 5-10), and the verso-reset branch never recognized this edition's convention of typesetting a chapter's own number in place of its literal verse-1 marker — both fixed and re-verified against zero drift across all 9 already-committed books (one real, non-gating correction surfaced in Alma's internal cross-reference file). A further hand fix corrected one page's chapter-boundary footnote mis-key (Malachi 4:1-2 at 3 Nephi 25-26) the walker's own picker couldn't resolve. Two cross-chapter misses caught after the fact: chapter 4's agent found a composite (Genesis 12:5 + 46:32-47:1) that belonged at 3:13 and wasn't there — added; chapter 7's figural link on Matthew 23:37 carried its own withdrawal condition ("if 3 Nephi 10:5 adjudicates independently") which chapter 10 then met on its own verbal evidence — withdrawn to the exclusion register per its own stated logic. **PLAN.md's claim that Frederick's weight concentrates at "3 Nephi 23, 2 Nephi 18, Moroni 12" is corrected above** — zero Frederick rows fall in 3 Nephi 23; his real weight is chapters 1, 5, 9, 11, 19, 20, 30, heaviest at 19 (the John 17 prayer). Wired into `data/books.ts`.

  **Findings that carry the book:** the Sermon at the Temple (12-14) is Matthew mediated against Luke on every tested point with zero exceptions (12 alone: 45 links across all 45 verses); the sixteen-chapter Isaiah 52 quotation (16, 20-21) shows "Father" substituted for "Lord" throughout and completes across chapters what it starts (16:20 drops Isaiah's possessive, 20:35 restores it as "the Father"); 3:13's proclamation gathering "their flocks and their herds, and all their substance" welds Genesis 12:5 to Genesis 46:32/47:1, two migrations for one wartime evacuation; 8:13's "smooth places" is `OT-via-earlier-BoM`, settling decisively against 1 Nephi 17:46 rather than the Bible; 9:19's "shedding of blood" abolishes, in Hebrews 9:22's own three words, the sacrifice that verse calls indispensable; 11:27/33-34 rest the strongest `kjvSpecific` claim in the corpus on KJV CANON rather than KJV wording — the Comma Johanneum and the longer ending of Mark; 18 institutes the sacrament in the vocabulary of the feeding miracles and Emmaus, not the Last Supper; 20:23/21:11 run the Deuteronomy 18 sanction in both its Acts form and its Deuteronomy form within one sermon, each carrying a Book-of-Mormon verb neither source has; 23:9 adds a detail ("should minister unto them") that is in neither of its two sources, in the very sentence rebuking an incomplete record; 26:4 is metaleptic on what it omits — Revelation 20:12's books, in a chapter arguing for an incomplete one; 28:6 takes as doctrine the reading of John 21:22-23 the Gospel itself retracts; and 29:7's "son of perdition" is Judas, recovered from John 17:12 and confirmed by the 1830 reading alone. The recurring open item, flagged independently by half a dozen chapters: the `mediation` enum's `OT-via-earlier-BoM` value has no clean name for an NT source reaching a verse through an earlier Book of Mormon passage — the same gap PLAN §7 already has open from the pilot, now hit repeatedly in a book that is one long chain of self-quotation.

  **The Hardy-apparatus bug (above) is the book's real methodological result:** a wrong `--next-heading` argument, not model error, produced ~90 rows that looked like unusually dense, ambiguous citations in a footnote-heavy sermon and were in fact a different book's footnotes entirely. Two independent dispatched agents (the first chapter-16 pass and, separately, the 13-15 batch reading the same raw data) both correctly diagnosed it as an apparatus artifact rather than adjudicating around it — which is the check `ADJUDICATING.md` §0's "never attribute a claim you can't verify" already asks for, applied one level up, to the apparatus itself rather than to a single citation.
